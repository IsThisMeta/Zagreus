...
class RadarrState extends ZebrraModuleState {
  RadarrState() {
    reset();
  }
  ...
  ZebrraListViewOption? _moviesViewType =
      RadarrDatabase.DEFAULT_VIEW_MOVIES.read();
  ZebrraListViewOption get moviesViewType => _moviesViewType!;
  set moviesViewType(ZebrraListViewOption moviesViewType) {
    ...
    if (_movies != null)
      _upcoming = _movies!.then((movies) {
        ...
        _missingOnly.forEach((movie) {
          if (movie.zebrraIsInCinemas && !movie.zebrraIsReleased)
            _notYetReleased.add(movie);
          if (!movie.zebrraIsInCinemas && !movie.zebrraIsReleased)
            _notYetInCinemas.add(movie);
        });
        _notYetReleased.sort((a, b) => a.zebrraCompareToByReleaseDate(b));
        _notYetInCinemas.sort((a, b) => a.zebrraCompareToByInCinemas(b));
    ...
    if (_movies != null)
      _missing = _movies!.then((movies) {
        List<RadarrMovie> _movies = movies.where((movie) {
          ...
          if (!movie.zebrraIsReleased) return false;
          return true;
        }).toList();
        _movies.sort((a, b) {
          int? _comparison;
          if (a.zebrraEarlierReleaseDate == null &&
              b.zebrraEarlierReleaseDate != null) return 1;
          if (b.zebrraEarlierReleaseDate == null &&
              a.zebrraEarlierReleaseDate != null) return -1;
          if (a.zebrraEarlierReleaseDate == null &&
              b.zebrraEarlierReleaseDate == null) _comparison = 0;
          _comparison ??=
              b.zebrraEarlierReleaseDate!.compareTo(a.zebrraEarlierReleaseDate!);
          if (_comparison == 0)
            return a.sortTitle!
                .toLowerCase()
