  ...
  Future<RadarrMovie?> addMovie({
    ...
    if (context.read<RadarrState>().enabled) {
      final result = await context
        ...
        if (showSnackbar)
          showZebrraErrorSnackBar(title: 'Failed to Add Movie', error: error);
        return RadarrMovie();
      });
  ...
  Future<bool> backupDatabase({
    ...
    if (context.read<RadarrState>().enabled) {
      return await context.read<RadarrState>().api!.command.backup().then((_) {
        if (showSnackbar)
          showZebrraSuccessSnackBar(
              title: 'Backing Up Database${ZebrraUI.TEXT_ELLIPSIS}',
              message: 'Backing up the database in the background');
        return true;
      }).catchError((error, stack) {
        ZebrraLogger().error('Unable to backup database', error, stack);
        if (showSnackbar)
          showZebrraErrorSnackBar(
  ...
  Future<bool> missingMovieSearch({
    ...
    if (context.read<RadarrState>().enabled) {
      return await context
        ...
        if (showSnackbar)
          showZebrraSuccessSnackBar(
              title: 'Searching${ZebrraUI.TEXT_ELLIPSIS}',
  ...
  Future<bool> runRSSSync({
    ...
    if (context.read<RadarrState>().enabled) {
      return await context.read<RadarrState>().api!.command.rssSync().then((_) {
        if (showSnackbar)
          showZebrraSuccessSnackBar(
              title: 'Running RSS Sync${ZebrraUI.TEXT_ELLIPSIS}',
              message: 'Running RSS sync in the background');
        return true;
      }).catchError((error, stack) {
        ZebrraLogger().error('Unable to run RSS sync', error, stack);
        if (showSnackbar)
          showZebrraErrorSnackBar(title: 'Failed to Run RSS Sync', error: error);
        return false;
      });
    }
    return false;
  ...
  Future<bool> updateLibrary({
    ...
    if (context.read<RadarrState>().enabled) {
      return await context
        ...
        if (showSnackbar)
          showZebrraSuccessSnackBar(
              title: 'Updating Library${ZebrraUI.TEXT_ELLIPSIS}',
              message: 'Updating library in the background');
        return true;
      }).catchError((error, stack) {
        ZebrraLogger().error('Unable to update library', error, stack);
        if (showSnackbar)
          showZebrraErrorSnackBar(
  ...
  Future<bool> deleteMovieFile({
    ...
    if (context.read<RadarrState>().enabled) {
      return await context
        ...
        if (showSnackbar)
          showZebrraErrorSnackBar(title: 'Failed to Delete File', error: error);
        return false;
      });
    }
    return false;
  ...
  Future<bool> updateMovie({
    ...
    if (context.read<RadarrState>().enabled) {
      return await context
        ...
        ZebrraLogger().error('Failed to update movie: ${movie.id}', error, stack);
        showZebrraErrorSnackBar(title: 'Failed to Update Movie', error: error);
        return false;
      });
    }
    return false;
  ...
  Future<bool> removeMovie({
    ...
    if (context.read<RadarrState>().enabled) {
      return await context
        ...
        ZebrraLogger().error('Failed to remove movie: ${movie.id}', error, stack);
        showZebrraErrorSnackBar(title: 'Failed to Remove Movie', error: error);
        return false;
      });
    }
    return false;
  ...
  Future<bool> quickImport({
    ...
    if (context.read<RadarrState>().enabled) {
      return await context
        ...
        if (showSnackbar)
          showZebrraSuccessSnackBar(
            title: 'Running Quick Import${ZebrraUI.TEXT_ELLIPSIS}',
            message: path,
          );
        return true;
      }).catchError((error, stack) {
        ...
        showZebrraErrorSnackBar(title: 'Failed to Quick Import', error: error);
        return false;
      });
    }
    return false;
  ...
  Future<bool> addTag({
    ...
    if (context.read<RadarrState>().enabled) {
      return await context
        ...
        ZebrraLogger().error('Failed to add tag: $label', error, stack);
        if (showSnackbar)
          showZebrraErrorSnackBar(
  ...
  Future<bool> deleteTag({
    ...
    if (context.read<RadarrState>().enabled) {
      return await context
        ...
        ZebrraLogger().error('Failed to add tag: ${tag.id}', error, stack);
        if (showSnackbar)
          showZebrraErrorSnackBar(title: 'Failed to Delete Tag', error: error);
        return false;
      });
    }
    return false;
  ...
  Future<bool> triggerManualImport({
    ...
    if (context.read<RadarrState>().enabled) {
      return await context
        ...
        ZebrraLogger().error('Failed to trigger manual import', error, stack);
        if (showSnackbar)
          showZebrraErrorSnackBar(title: 'Failed to Import', error: error);
        return false;
      });
    }
    return false;
