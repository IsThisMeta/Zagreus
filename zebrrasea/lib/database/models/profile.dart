...
@JsonSerializable()
@HiveType(typeId: 0, adapterName: 'ZebrraProfileAdapter')
class ZebrraProfile extends HiveObject {
  static const String DEFAULT_PROFILE = 'default';

  static ZebrraProfile get current {
    final enabled = ZebrraSeaDatabase.ENABLED_PROFILE.read();
    return ZebrraBox.profiles.read(enabled) ?? ZebrraProfile();
  }

  static List<String> get list {
    final profiles = ZebrraBox.profiles.keys.cast<String>().toList();
  ...
  factory ZebrraProfile({
    ...
    return ZebrraProfile._internal(
      // Lidarr
  ...
  Map<String, dynamic> toJson() {
    final json = _$ZebrraProfileToJson(this);
    json['key'] = key.toString();
    return json;
  }

  factory ZebrraProfile.fromJson(Map<String, dynamic> json) {
    return _$ZebrraProfileFromJson(json);
  }

  factory ZebrraProfile.clone(ZebrraProfile profile) {
    return ZebrraProfile.fromJson(profile.toJson().cast<String, dynamic>());
  }

  factory ZebrraProfile.get(String key) {
    return ZebrraBox.profiles.read(key)!;
  }

  bool isAnythingEnabled() {
    for (final module in ZebrraModule.active) {
