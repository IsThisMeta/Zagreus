...
import 'package:zebrrasea/system/webhooks.dart';

class RadarrWebhooks extends ZebrraWebhooks {
  @override
  Future<void> handle(Map data) async {
    _EventType? event = _EventType.GRAB.fromKey(data['event']);
    if (event == null)
      ZebrraLogger().warning(
...
extension _EventTypeExtension on _EventType? {
  ...
  Future<void> _testEvent(Map data) async => ZebrraModule.RADARR.launch();
  ...
  Future<void> _goToMovieDetails(int? movieId) async {
    ...
    return ZebrraModule.RADARR.launch();
  }
}
