...
class ZebrraColours {
  /// List of ZebrraSea colours in order that the should appear in a list.
  ///
...
extension ZebrraColor on Color {
  Color disabled([bool condition = true]) {
    if (condition) return this.withOpacity(ZebrraUI.OPACITY_DISABLED);
    return this;
  }

  Color enabled([bool condition = true]) {
    if (condition) return this;
    return this.withOpacity(ZebrraUI.OPACITY_DISABLED);
  }

  Color selected([bool condition = true]) {
    if (condition) return this.withOpacity(ZebrraUI.OPACITY_SELECTED);
    return this;
  }

  Color dimmed() => this.withOpacity(ZebrraUI.OPACITY_DIMMED);
}
